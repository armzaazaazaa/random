...
arrLabal['key'] = 'key';
arrLabal['name'] = 'ชื่อ';
arrLabal['company'] = 'บริษัท';
arrLabal['title'] = 'ตำแหน่ง';
arrLabal['active_status'] = 'สถานะ';
arrLabal['parent'] = 'หัวหน้า';
var arraylist = ["ใช้งาน", "ไม่ใช้งาน", "สรรหา", "ปิดสรรหา"];
var data;
...
Inspector.prototype.buildPropertyRow = function(propertyName, propertyValue) {
    ...
    if ( /*propertyName!==''&&*/ propertyName !== 'idPosition' && propertyName !== 'company') {
        td1.textContent = arrLabal[propertyName];
        tr.appendChild(td1);
    }

    var td2 = document.createElement("td");
    if (propertyName == 'active_status') {
        var input = document.createElement("select");
        input.id = "active_status";
        input.setAttribute('style', 'color: #000000;width:100%');
        for (var i = 0; i < arraylist.length; i++) {
            var option = document.createElement("option");
            option.setAttribute('style', 'color: #000000');
            option.value = i;
    ...
    //input.setAttribute("id", 'inputEdit' + this.tabIndex);
    input.setAttribute("id", propertyName);
    var decProp = this.declaredProperties[propertyName];
    input.tabIndex = this.tabIndex++;
    var availableTags = [];
    data['company'] = $('#selectworking').val();
    $.get('getautoposition', { data: data['company'] }, function(res) {

        $.each(res, function(index, val) {
            var label = val.Name+ "("+val.PositionCode+")";
            availableTags.push({ label: label, value: label, text: val.id });
        });
    ...
    if (decProp !== undefined && decProp.hasOwnProperty("type")) {
        if (decProp.type === 'hidden') {
